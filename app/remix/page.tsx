...
// Add TypeScript interface for IP details
interface IPDetails {
  id: string;
  title: string;
  description: string;
...
function IPDetailsFromParams({ onIPLoad }: { onIPLoad: (ip: IPDetails) => void }) {
  const searchParams = useSearchParams()
  
  useEffect(() => {
    const selectedIP: IPDetails = {
...
export default function RemixPage() {
  ...
  useEffect(() => {
    if (!ipDetails) return;
    const ethTotal = ipDetails.pricePerDay * duration;
    setTotalPrice(ethTotal);
    setTotalPriceUSD(ethTotal * ethPrice);
  }, [duration, ipDetails?.pricePerDay, ethPrice, ipDetails]);
  ...
  return (
    <GradientBackground>
      <Suspense fallback={<div className="flex items-center justify-center min-h-screen">Loading...</div>}>
        <IPDetailsFromParams onIPLoad={setIPDetails} />
      </Suspense>
            ...
            <div className="grid gap-8 md:grid-cols-2">
              ...
              <GlassCard>
                <div className="p-4 bg-gradient-to-r from-primary/20 to-primary/10 rounded-t-md border-b border-white/10">
                  <h2 className="text-xl font-semibold text-center">{ipDetails.title}</h2>
                  <p className="text-sm text-center text-white/70 mt-2">{ipDetails.type}</p>
                </div>
                <div className="p-4">
                  <p className="text-sm text-white/70 mb-4">{ipDetails.description}</p>
                
                  <div className="space-y-2 border-t border-white/10 pt-4 mb-4">
                    <div className="flex items-center justify-between text-sm">
                      <span className="text-white/60">Owner</span>
                      <span>{ipDetails.owner}</span>
                    </div>
                    <div className="flex items-center justify-between text-sm">
                      <span className="text-white/60">Type</span>
                      <span>{ipDetails.type}</span>
                    </div>
                    <div className="flex items-center justify-between text-sm">
                      <span className="text-white/60">License</span>
                      <span>{ipDetails.license}</span>
                    </div>
                    <div className="flex items-center justify-between text-sm">
                      <span className="text-white/60">Royalty</span>
                      <span>{ipDetails.royalty}%</span>
                    </div>
                    <div className="flex items-center justify-between text-sm">
                      <span className="text-white/60">Created</span>
                      <span>{ipDetails.creationDate}</span>
                    </div>
                    <div className="flex items-center justify-between text-sm">
                      <span className="text-white/60">License Fee</span>
                      <div className="text-right">
                        <div>{ipDetails.pricePerDay} ETH per day</div>
                        <div className="text-xs text-white/40">≈ ${(ipDetails.pricePerDay * ethPrice).toFixed(2)} USD</div>
                      </div>
                    </div>
                  </div>
                ...
                <div className="space-y-4 mb-6">
                  ...
                  <div className="border-t border-white/10 pt-4">
                    <div className="flex justify-between font-medium mb-2">
                      ...
                      <div className="text-right">
                        <div>{ipDetails.pricePerDay} ETH</div>
                        <div className="text-xs text-white/40">≈ ${(ipDetails.pricePerDay * ethPrice).toFixed(2)} USD</div>
                      </div>
                    </div>
                  ...
                  <div className="p-3 bg-primary/10 border border-primary/20 rounded-md">
                    ...
                    <ul className="text-xs space-y-1 text-white/70">
                      ...
                      <li className="flex items-center">
                        <Zap className="h-3 w-3 mr-1 text-primary" /> {ipDetails.royalty}% royalty on commercial use
                      </li>
                    </ul>
                  </div>
                </div>
      ...
      <Dialog open={isGuideLinesOpen} onOpenChange={setIsGuidelinesOpen}>
        <DialogContent className="max-w-2xl">
          <DialogHeader>
            <DialogTitle>
              Usage Guidelines for {ipDetails?.title || 'IP Asset'}
            </DialogTitle>
            <DialogDescription>
          ...
          <div className="mt-4 space-y-4">
            ...
            <div className="space-y-2">
              ...
              <pre className="bg-black/30 p-2 rounded text-xs mt-2 overflow-x-auto">
                "Original work: {ipDetails?.title} by {ipDetails?.owner}, licensed through CollabVerse."
              </pre>
            </div>
            ...
            <div className="space-y-2">
              ...
              <p className="text-sm text-white/70">
                A {ipDetails?.royalty}% royalty fee applies to all commercial revenue generated from the use 
